...
@section('content')
    ...
    <div class="px-4 sm:px-6 lg:px-8 py-28 bg-stone-100">
        <div class="w-[90%] mx-auto">
                ...
                @foreach ($beauticians as $beautician)
                    <div class="bg-white rounded-md shadow-md overflow-hidden" data-aos="fade-up" data-aos-duration="1000"
                        ...
                        <div class="mt-5 flex flex-col px-5">
                            <span class="text-xs text-stone-800 poppins-medium uppercase">Hairstylist</span>
                            <h1 class="text-lg mt-2 text-stone-700 poppins-semibold">{{ $beautician->name }}</h1>
                            {{-- <p class="text-sm text-stone-600">
                                {{ $beautician->description }}
                            </p>  --}}
                            {{-- <div class="mt-4 grid grid-cols-4 pb-5">
                                <a href="{{ $beautician->facebook_link }}" target="_blank"
                                    class="bg-stone-700 text-white flex justify-center items-center w-12 h-12 rounded-full">
                                    <i class="fa-brands fa-facebook-f"></i>
                                </a>
                                <a href="{{ $beautician->instagram_link }}" target="_blank"
                                    class="bg-stone-700 text-white flex justify-center items-center w-12 h-12 rounded-full">
                                    <i class="fa-brands fa-instagram"></i>
                                </a>
                                <a href="{{ $beautician->twitter_link }}" target="_blank"
                                    class="bg-stone-700 text-white flex justify-center items-center w-12 h-12 rounded-full">
            ...
            {{-- Therapist --}}
            <h1 class="text-4xl text-stone-700 text-center poppins-semibold my-20">
                Our Therapist
            </h1>
            <div class="grid grid-cols-1 lg:grid-cols-4 gap-12">
                @foreach ($beauticians as $beautician)
                    <div class="bg-white rounded-md shadow-md overflow-hidden" data-aos="fade-up" data-aos-duration="1000"
                        ...
                        <img src="/uploads/beauticians/{{ $beautician->image }}" class="w-full h-[350px] object-cover">
                        <div class="mt-5 flex flex-col px-5">
                            <span class="text-xs text-stone-800 poppins-medium uppercase">Therapist</span>
                            <h1 class="text-lg mt-2 text-stone-700 poppins-semibold">{{ $beautician->name }}</h1>
                        </div>
                    </div>
                @endforeach
